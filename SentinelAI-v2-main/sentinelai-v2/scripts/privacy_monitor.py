...
def print_startup_banner(browser: str):
    print("\n" + "═" * 56)
    print("       🔍  AI Privacy Monitor  —  Now Watching")
    print("═" * 56)
    print(f"  Your browser  :  {browser.title()}")
    print(f"  Started at    :  {datetime.now().strftime('%I:%M %p')}")
    print()
    print("  This tool watches every website your device contacts")
    print("  in the background — including hidden trackers that")
    print("  you never opened or agreed to.")
    print()
    print("  Press Ctrl+C at any time to stop and see a summary.")
    print("═" * 56 + "\n")

    if browser in ("chrome", "firefox", "edge"):
        print("  ⚠  IMPORTANT — One setting to check first:")
        print("     Your browser may be hiding some tracker activity.")
        print("     To make sure this tool sees everything, turn off")
        print("     'Secure DNS' in your browser:")
        if browser == "chrome":
            print("     Chrome  →  Settings → Privacy & Security")
            print("                → Security → Use Secure DNS → Turn OFF")
        elif browser == "firefox":
            print("     Firefox →  Settings → General → Network Settings")
            print("                → Uncheck  'Enable DNS over HTTPS'")
        elif browser == "edge":
            print("     Edge    →  Settings → Privacy, Search & Services")
            print("                → Security → Use Secure DNS → Turn OFF")
        print()
    print("  Watching your traffic now...\n")
