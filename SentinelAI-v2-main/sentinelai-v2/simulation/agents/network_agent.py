...
class NetworkAgent:
    ...
    def attack(
        ...
        # Strip HTTP scheme for modules that expect a hostname
        ...
        stats, success, evidence, why_failed = self._run_module(
            attack_type, host, self.port, merged_opts
        )
    ...
    def _run_module(
        ...
        try:
            ...
            logger.warning("Cannot import %s: %s — using simulation", mod_path, e)
        ...
        if exc_holder:
            logger.warning("Module %s raised: %s — simulating", attack_type, exc_holder["err"])
            return self._simulate_stats(attack_type), *self._simulate_outcome(attack_type)
