...
Real C2 malware beacons at regular intervals with slight jitter.
Signature: same IP sends 5+ POST /beacon requests whose inter-request
intervals have a Coefficient of Variation (CV = σ/μ) below 0.55
— i.e. the timing is suspiciously regular.
"""
...
MIN_BEACONS = 5
TIME_WINDOW = 120.0   # look back 2 minutes
CV_THRESHOLD = 0.55   # low CV → periodic → suspicious
    ...
    def analyse(self) -> None:
        ...
        for evt in state.recent_events(TIME_WINDOW):
            if evt.event_type != "http_request":
                continue
            # Match beacon or generic POST (C2BeaconSimulator hits root or /beacon)
        ...
        for ip, timestamps in ts_by_ip.items():
            ...
            if mean < 0.5:   # too fast — probably a flood, not a beacon
                continue
