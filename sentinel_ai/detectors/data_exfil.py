...
Signature: same IP POSTs binary data (application/octet-stream) totalling
>20 KB OR 6+ such requests within 15 s.
"""
    ...
    def analyse(self) -> None:
        ...
        for evt in state.recent_events(TIME_WINDOW):
            if evt.event_type != "http_request" or evt.method != "POST":
                continue
            # Match octet-stream (binary dump) or any large POST to /upload or /data
            is_binary = "octet-stream" in evt.content_type
            is_large  = evt.content_length > 512
